...
- `blog/claim`'s relationship arrays (`supports`/`opposes`/`assumes`/`refines`/`contradicts`) and `sources` are carried as one opaque, untranslated, kind-owned frontmatter fragment (produced once by `ClaimPublicationKind` at admission time), not machine-translated and not modeled as generic `PublicField`s.

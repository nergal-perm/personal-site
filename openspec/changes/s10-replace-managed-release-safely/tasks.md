...
replaces the prior managed RU/EN markdown files and provenance record atomically with respect to other
installers as one coherent generation; input drift between planning and commit blocks the release with live
trees untouched; an interruption recovers deterministically on the next retry using a provenance-vs-tree
joint completion marker spanning both locale files (per `design.md` D2's final, twice-corrected policy — NOT
an unconditional "always roll back to old"); concurrent replacement attempts for the same identity are
serialized via a real cross-process lock.
...
protocol for `ru.md`/`en.md`, with provenance written last as the joint completion marker for both files
together (not per-file independently — an earlier design draft that checked each locale independently could
produce a mixed new-RU/old-EN generation, corrected before implementation completed). Durable recovery
(`recoverIfNeeded`) compares the current whole-site provenance against the current tree; on a mismatch with a
backup present, it restores from backup and retains the backup until the provenance rewrite itself succeeds
(backups are deleted only after a successful provenance commit, so a provenance-write failure during recovery
itself remains recoverable on a later retry). The existing `Files.createFile(...)`-based install lock is
replaced with `FileChannel.tryLock()`, exactly mirroring S09's final, `SIGKILL`-probe-verified design.
`InstallToSiteHandler` recaptures and re-verifies the approved snapshot's hashes immediately before commit
(input-drift guard, D4), holding the approval lock across both the recheck and the site commit per the final
review's Important-1 fix. No new production adapter — this is entirely inside the existing
`site`/`installtosite` packages.
...
- [x] 1.1 **Write the failing unit test proving replace-not-block in `NullManagedSiteInstallerTest`**
...
- [x] 1.2 **Run it to confirm it fails** (throws `SiteAlreadyInstalledException` today)

Run: `cd publication-exporter && mvn -q -Dtest=NullManagedSiteInstallerTest test`
Expected: FAILURE.

- [x] 1.3 **Remove the create-only guard from `NullManagedSiteInstaller.install(...)`**
...
- [x] 2.1 **Read `InstallToSiteCliAcceptanceTest` and `InstallToSiteCommand` fully first** to learn the exact
...
- [x] 2.2 **Write one failing acceptance test**: `secondInstallToSiteReplacesTheManagedGeneration()` — GIVEN a
...
- [x] 3.1 **Read the current `"replacing it is not yet supported"` call site in `installApprovedSnapshot(...)`**
...
- [x] 3.2 **Add the input-drift recheck (D4) immediately before calling `managedSiteInstaller.install(...)`**
...
- [x] 3.3 **Add/update `InstallToSiteHandlerTest` cases**: a replace-succeeds case (unchanged approved
...
- [x] 3.4 **Run the acceptance test and full suite**
...
- [x] 4.1 **Write a failing test proving replacement + durable recovery for a single locale file, mirroring
...
- [x] 4.2 **Run it to confirm it fails** (no per-file backup/restore or recovery logic yet — the old code
...
- [x] 4.3 **Implement the per-file backup/restore replace protocol for `installManagedGeneration(...)`**,
...
- [x] 4.4 **Replace the install lock with `FileChannel.tryLock()`**, mirroring
...
- [x] 4.5 **Implement `recoverIfNeeded(identity)` per `design.md` D2's final policy (provenance-vs-tree joint
  completion marker, backups retained until provenance commit succeeds), called at the top of `install(...)`
  before staging/replacing. The code snippet below reflects the task's original first-draft policy; the
  implementer corrected it twice more during review — see `progress.md` and `design.md` D2 for the final
  shape actually committed.**
...
- [x] 4.6 **Run the tests from 4.1, plus the full class, plus Task 2's CLI acceptance test**
...
- [x] 4.7 **Write and pass a second acceptance-level test: two release generations through the shared
...
- [x] 5.2 **Manually trace each requirement scenario to its covering test(s)**: REL-03 "Same approved state is

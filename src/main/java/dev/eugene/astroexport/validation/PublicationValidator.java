...
public final class PublicationValidator {
  ...
  public List<PublicationDiagnostic> validate(Map<String, Object> frontmatter, String body) {
    if (!Boolean.TRUE.equals(frontmatter.get("publish"))) {
      return List.of(new PublicationDiagnostic("publish", "must be true; allowed value: true"));
    }
    ...
    if (collection == null || !PublicationKind.allowedCollections().contains(collection)) {
    ...
    if (contentType == null || !PublicationKind.allowedContentTypes(collection).contains(contentType)) {
  ...
  private static String maskProtectedContexts(String body) {
    ...
    for (String line : body.split("(?<=\\n)", -1)) {
      ...
      if (fenced || trimmed.startsWith("```") || trimmed.startsWith("~~~")) {
        ...
        result.append(line.replaceAll("(?s)<!--.*?-->", " ").replaceAll("(?s)%%.*?%%", " "));
      }
    }

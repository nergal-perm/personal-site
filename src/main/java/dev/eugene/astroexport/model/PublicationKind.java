...
public enum PublicationKind {
  ...
  private static List<PublicationRequirement> selectionRequirements() {
    return List.of(
        requirement(List.of("publish"), "must be true", "логическое значение `true`; намерение публиковать задаёт только автор", "frontmatter", RequirementValidator.BOOLEAN_TRUE),
        requirement(List.of("publicId"), "lowercase route slug", "уникальный slug из строчных латинских букв, цифр и внутренних дефисов", "frontmatter", RequirementValidator.NON_EMPTY_STRING),
        requirement(List.of("publicCollection"), "must match the publication collection", "точное имя коллекции из заголовка этого раздела", "frontmatter", RequirementValidator.SUPPORTED_COLLECTION),
        requirement(List.of("publicContentType"), "must match the publication kind", "точный тип материала из заголовка этого раздела", "frontmatter", RequirementValidator.SUPPORTED_CONTENT_TYPE));
  }
